...
## [1.74.2] - 2026-07-29 "Herald"

### Fixed
- **"Candidato não encontrado" no perfil do Banco de Talentos** — `empresa/CandidateProfile.tsx` resolvia o candidato com `useCandidates(undefined, { page: 1, pageSize: 1000 })` (ordenado por `created_at desc`) e filtrava por id no client com `.find()`. Com 1088 candidatos na base, quem não estivesse entre os 1000 mais recentes caía no estado de "não encontrado" ao abrir o perfil — mesmo aparecendo normalmente na listagem, que já havia sido corrigida no #34. Substituído pelo hook `useCandidate(id)`, que busca o registro diretamente por id (`src/pages/empresa/CandidateProfile.tsx`) (#36)
- **Teto de 1000 candidatos em 8 lookups client-side** — o mesmo anti-padrão (`pageSize: 1000` + join por id no client) sobrevivia em outras oito telas/hooks, quebrando dados reais: 147 candidaturas (46 candidatos distintos), 36 conversas, 6 entrevistas e 3 favoritos referenciavam candidatos fora da janela de 1000. Sintomas confirmados em produção: conversas renderizando sem nome nem avatar em Mensagens (o `messagesService` devolve `candidateName: ''` de propósito, então a lista capada era a única fonte, sem fallback) e a tela "Candidatos Salvos" exibindo "Você ainda não salvou nenhum candidato" porque `getFavoriteCandidates()` descartava no `.find()` o favorito fora do teto. Adicionados `getCandidatesByIds()` ao service (busca em lote por id, em chunks de 100 para não estourar o limite de URL do PostgREST — mesmo tratamento já aplicado em `getSkillsForCandidates` na 1.73.1) e o hook `useCandidatesByIds()` (ids deduplicados e ordenados para estabilizar a chave de cache), usados nas cinco telas de lookup e em `useFavoriteCandidates`. Nos hooks de recomendação a correção seguiu o que o motor de fato consome: `useJobRecommendations` passou a usar `useCandidate(id)` (`getRecommendedJobs` só faz `data.candidates.find(c => c.id === candidateId)`, ou seja, listava a base inteira para achar um único candidato) e `useCandidateRecommendations` passou a usar `useAllCandidates()` (pontua todos os candidatos contra a vaga via `applyExclusionFilters`, então precisa mesmo do dataset completo, agora drenado sem teto) (`src/services/candidates/candidatesService.ts`, `src/services/candidates/candidatesService.supabase.ts`, `src/hooks/useCandidatesQuery.ts`, `src/hooks/useFavoriteCandidates.ts`, `src/hooks/useCandidateRecommendations.ts`, `src/hooks/useJobRecommendations.ts`, `src/pages/empresa/Applications.tsx`, `src/pages/empresa/Messages.tsx`, `src/pages/empresa/Interviews.tsx`, `src/pages/empresa/JobTestManager.tsx`, `src/pages/admin/JobDetail.tsx`) (#37)

### Changed
- **N+1 de habilidades padronizadas na lista de Candidaturas** — `empresa/Applications.tsx` montava um `useQueries` com uma requisição de skills por candidato de `allCandidates`, ou seja, ~1000 queries paralelas para uma tela que só precisa dos candidatos com candidatura na empresa. Com o lookup passando a ser por id, o fan-out ficou restrito a quem realmente se candidatou (`src/pages/empresa/Applications.tsx`) (#37)
